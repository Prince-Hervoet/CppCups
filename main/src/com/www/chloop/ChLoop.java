...
public class ChLoop {
    ...
    public void addNode(MappingNode mn) {
        ...
        for (int i = 0; i < nodeSize; i++) {
            long hash = HashUtil.hash(mn.key + "-" + i);
            int targetId = (int) (hash % FLAG);
            nodeMap.put(targetId, mn);
            ids[i] = targetId;
        }
        int[] copyList = new int[nodeSize + nodes.length];
